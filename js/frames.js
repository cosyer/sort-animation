...
Frames.prototype.exchangeSquare = function (i, j, isAdd = true) {
  ...
  this.squares[i].css.transform =
    "translate(" +
    (j - this.arr.length / 2 + 0.5) * 2.5 +
    "rem, " +
    this.squares[i].positiony * (50 + this.parentHeight) +
    "px)";
  this.squares[j].css.transform =
    "translate(" +
    (i - this.arr.length / 2 + 0.5) * 2.5 +
    "rem, " +
    this.squares[j].positiony * (50 + this.parentHeight) +
    "px)";
...
Frames.prototype.moveSquare = function (i, j, isAdd = true) {
  ...
  if (this.squares[i].positiony) {
    this.squares[i].css.transform =
      "translate(" + (j - this.arr.length / 2 + 0.5) * 2.5 + "rem" + ") ";
...
Frames.prototype.changePositiony = function (i, isAdd = true) {
  ...
  if (this.squares[i].positiony) {
    ...
    this.squares[i].css.transform =
      "translate(" +
      (this.squares[i].positionx - this.arr.length / 2 + 0.5) * 2.5 +
      "rem" +
      ")";
  } else {
    this.squares[i].css.transform =
