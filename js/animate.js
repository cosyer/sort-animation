function Animation(json) {
  ...
  this.onstart = json.onstart
    ? json.onstart
    : function () {
        return null;
      };
  this.onstop = json.onstop
    ? json.onstop
    : function () {
        return null;
      };
  this.onover = json.onover
    ? json.onover
    : function () {
        return null;
      };
...
Animation.prototype.setSpeed = function (speed) {
  ...
  if (this.frames[0]) {
    for (var i = 0; i < this.frames[0].length; i++) {
      $(this.frames[0][i].el).css(
        "transition",
        "transform " + this.interval / 1000 + "s linear"
      );
    }
  }
};
...
Animation.prototype.update = function () {
  ...
  for (var i = 0; i < frame.length; i++) {
    if (
      String($(frame[i].el)[0].style.transform) !==
      String(frame[i].css.transform)
    ) {
      this.isNotmove = false;
    }
